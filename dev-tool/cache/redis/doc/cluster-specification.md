...
本节将说明如何使用epoch概念升级从节点并在升级过程抵抗分区。

* 主节点不可达，主节点有三个从节点A、B、C
* 从节点A赢得选举被提升为主节点
* 由于网络分区，集群中的多数节点不能访问到A
* 从节点B赢得选举被提升为主节点
* 由于分区，B不能被集群中的大多数节点访问
* 前一个分区恢复，A又可用

此时B关闭，A再次可用，角色为master(实际上，更新消息会立即重新配置它，但这里我们假设所有更新消息都丢失了)。此时为了对B进行failover，从节点C开始发起选举请求。

1. C尝试选举并获得成功，configEpoch将递增

2. A将不能声称是其slot的主节点，因为持有相同的slot的节点与A节点相比有更高的configEpoch

3. 所有节点将更新它们的配置，将slot分配给C节点，集群继续提供服务

正如你将在下一节中看到的，重新加入群集的陈旧节点通常会尽快收到配置更改的通知，因为只要它ping任何其它节点，接收者就会检测到它具有陈旧信息并发送更新消息。

### hast slot配置传播

Redis集群的一个重要组成部分是用来传播关于哪些节点负责哪些slot的机制。这对于新集群的配置和从节点进行failover后更新配置都是非常重要的。

同样的机制允许被分区的节点在不确定的时间内以合理的方式重新加入集群。

hash slot配置有两种传播方式:

1. 心跳消息。ping或pong数据包的发送节点总是添加有关它（或其主节点的，如果它是从节点）负责的slot信息。

2. 更新消息。在每个heartbeat包中都有关于发送节点的configEpoch的信息和负责的salt，如果heartbeat包的接收方发现发送方信息过时，它将发送一个包含新信息的包，迫使过时节点更新其信息。

heartbeat或UPDATE消息的接收者使用某些简单的规则来更新其上的节点与slot的映射关系。

当创建一个新的Redis集群节点时，它的本地哈希槽表被简单的初始化为NULL，这样每个slot就不会绑定或链接到任何节点。这看起来类似于:
...
为了更新其slot表，节点遵循的第一条规则如下:

Rule 1:如果哈希槽未分配(设置为空)，并且有已知节点声明负责这些slot，将修改哈希槽表并将这些slot与这个节点关联。

如果我们从A节点收到心跳信息，声明其负责配置epoch为3的slot1和slot2，节点的slot映射关系将被修改为:
...
创建新集群时，系统管理员需要手动分配(使用cluster ADDSLOTS命令、通过redis trib命令行工具或任何其它方式)每个主节点负责的slot，这些配置信息将在集群中迅速传播。

仅有这条规则是不够的，我们知道slot的映射表在两种情况下会发生变化:

1. 从节点升级为主节点
2. 重新分片时slot移动到另一个节点

现在让我们先看下failover。当从节点进行failover时，它先获取到一个配置epoch，这个配置epoch一定是大于它的主节点的(通常大于其它任务先前的配置)。例如，作为A的从节点的B，可以使用配置epoch为4的B进行故障转移。它将发送心跳包，由于下面的第二条规则，接受者将更新slot配置:

Rule 2: 如果slot已经分配，并且已知节点正在使用大于当前与该slot关联的主节点configEpoch的configEpoch进行广播，则将这些slot重新绑定到这个节点。

因此，在从B接收到声称服务于配置epoch为4的散列槽1和2的消息后，接收者将按以下方式更新slot配置：
...
由于第二条规则，最终集群中的所有节点都会同意，configEpoch最大的节点拥有slot。

Redis集群中的这个机制被称为last failover wins。

重新分片的过程也是如此。当导入slot的节点完成了操作，其配置epoch将递增，以确保更改能在整个集群中传播。

### 更新消息

节点A可能在一段时间后重新加入群集，它将发送心跳包声名负责配置epoch为3的slot1和slot2,其它接收者收到更新消息后将slot与具有更新的配置epoch的节点B关联。

### 节点如何重新加入集群

节点重新加入集群时，使用想用的基本机制。继续上面的示例，将通知节点A的slot1和slot2现在有节点B负责。加入A只负责这两个slot，现在A不在负责任何slot，因此A将重新配置成为新master的从节点。

实际情况可能比这复杂，通常在一段时间后A才重新加入集群，同时A原本负责的slot可能被分配到多个节点，如slot1分配给B，slot2分配给C。

Redis集群节点角色切换的规则:主节点A将改变配置成为现在负责这些slot的节点的从节点。从节点也将重新配置成为新的主节点的从节点。

### 副本迁移

Redis集群实现了一个称为副本迁移的概念，以提高系统的可用性。其思想是，在具有主从配置的集群中，如果从节点与主节点之间映射是固定，随着时间的推移可用性是有限的，单个节点可能会发生故障。

例如，在每个主节点都有一个从节点的集群中，当有一个主节点或从节点发生故障时，集群还能正常工作，但是当同时失败时集群将不能正常工作。

如果主从关系是固定不变的，那么使集群提高抵抗上述问题的方法只有向每个主节点添加更多从节点，但是成本很高需要更多的Redis实例更多的内存等等。

另一种方法是创建一个非对称性集群，并让集群布局随时间自动调整。例如集群有3个主节点A、B、C，A和B各有一个从节点A1和B1，C有两个从节点C1和C2。

复本迁移是自动重新配置的过程，将从节点迁移到没有从节点覆盖的主节点。通过副本迁移，上面提到的场景将变成如下过程:

* 主节点A失败，A1被升级
* C2被迁移作为没有任何从节点覆盖的A1的从节点
* 三小时后A1又失败
* C2被提升为新主节点替代A1
* 集群继续工作
...
迁移算法不使用任何形式的协议，因为Redis集群中的从节点布局不属于集群配置的一部分，所以不需要考虑配置的一致性。相反，它使用一种算法来避免当主节点没有备份时从节点大规模迁移。该算法保证最终每个主节点将至少有一个从节点作为备份。

首先我们需要定义在这个上下文中什么是好的从节点:从给定的节点角度来看，好的从节点是不处于FAIL状态的从节点。

当从节点检测到有主节点没有好的从节点时将触发副本迁移算法。但是在所有检测到这种情况的从节点中，通常只有一个从节点能执行迁移算法。要从拥有最多好的从节点下且node id最小的主节点下选择从节点。

例如，这里有一个10个主节点每个都有一个从节点，2个主节点有5个从节点，尝试迁移时会从拥有5个从节点中的主机点中选取node id最小的主节点下的从节点。既然没有协议，当集群的状态不稳定时可能会出现多个从节点认为自己具有较低节点ID的无障碍的从节点。如果发生这种情况，多个从节点会迁移到主节点下，这是无害的。如果竞争的出现将主节点下的所有从节点都被迁移走，一旦集群稳定算法将再次执行并将从节点迁移回主节点。

算法由用户配置的参数cluster-migration-barrier控制:主节点下的好的从节点数量必须达到这个阀值才能迁移。

### configEpoch冲突解决算法

从节点进行failover时，将生成一个新的configEpoch值(递增)并保证唯一。

1. CLUSTER FAILOVER命令使用TAKEOVER可以手动进行failover,不用管是否大多数主节点同意

2.由于重新分片导致的迁移也会在本地节点生成一个configEpoch

特别的，当手动重新分片时，当slot从A节点迁移到B节点，将强制B更新为集群中最大的epoch加1，不需要其它节点的同意。通常实际场景中需要移动几百个slot(特别是小集群)。在移动每个slot时都协定生成一个新的configEpoch是低效的。此外每个节点都需要同步保存新的配置。由于它的执行方式，我们只需要在第一个slot移动时生成一个新的configEpoch，这使得它在生产环境的更高效。
//待定
